...
import '../css/bkflow.styl';
import './template-web.js';
...
(function ($, artTemplate) {
  $.fn.extend({
    dataflow: function (options) {
      ...
      // 完成初始化
      window.jsPlumb.fire('jsPlumbDemoLoaded', instance);

      // flow插件初始化
      var init = function () {
        window.jsPlumb.setContainer(opts.canvas);

          // 将所有节点包裹在内置的DIV
        $(opts.canvas).append('<div class="flow-container"></div>');
        $nodeContainer.el = $(opts.canvas).find('.flow-container');

          // 加入基础样式class
        $(opts.tools).addClass('flow-tool');
        _self.addClass('flow-wrap');
        $(opts.canvas).addClass('flow-canvas');
      ...
      var _initDragging = function () {
        ...
        function canvasDragHanlder (e) {
          ...
          if (opts.dragSelection) {
            ...
            _self.find('.flow-container').append(selectWrapper);
        ...
        function canvasMoveEndHandler (e) {
          console.log('--------------canvasMoveEndHandler--------------------');
              // 拖拽结束更新画布的位移值
        ...
        function selectionMoveHanlder (e, posLeft, posTop) {
          console.log('--------------selectionMoveHanlder--------------------');
        ...
        function selectionEndHandler (e) {
          console.log('--------------selectionEndHandler--------------------');
        ...
        function isNodeInSelectionArea ($node) {
          console.log('--------------isNodeInSelectionArea--------------------');
        ...
        function locationLineCopyHandler (e) {
          console.log('--------------locationLineCopyHandler--------------------');
        ...
        function locationLinePasteHandler (e) {
          console.log('--------------locationLinePasteHandler--------------------');
        ...
        function locationLineDeleteHandler (e) {
          console.log('--------------locationLineDeleteHandler--------------------');
        ...
        function toolsMoveEndHandler (e) {
          console.log('--------------toolsMoveEndHandler--------------------');
        ...
        $(opts.canvas).on('mouseup', '.bk-flow-location', function (e) {
          console.log('--------------.bk-flow-location mouseup--------------------');
        ...
        $(opts.canvas).on('click', '.bk-flow-location', function (e) {
          console.log('--------------.bk-flow-location click--------------------');
          e.stopPropagation();
          ...
          console.log('--------------.branch-condition click--------------------');
          e.stopPropagation();
          if (opts.isEdit) {
            const $this = $(e.target);
      ...
      var _connection_to_line = function (conn) {
        console.log('--------------_connection_to_line--------------------');
      ...
      var _disabledEdit = function () {
        console.log('--------------_disabledEdit--------------------');
        opts.isEdit = false;

        let el = document.querySelectorAll(opts.canvas + ' .bk-flow-location');
      ...
      var _createLocation = function (location) {
        ...
        if (opts.isEdit) {
              // 使得节点可拖拽
          var locationEl = window.jsPlumb.getSelector(opts.canvas + ' ' + `#${location.id}`);
      ...
      var _updateLocationeById = function (id, data) {
        console.log('--------------_updateLocationeById--------------------');
        var $location = $('#' + id);
      ...
      var _getConnectionsByNodeId = function (id, type) {
        console.log('--------------_getConnectionsByNodeId--------------------');
      ...
      */
      ...
      dataflow.prototype.updateLocationById = function (id, data) {
        console.log('--------------updateLocationById--------------------');
